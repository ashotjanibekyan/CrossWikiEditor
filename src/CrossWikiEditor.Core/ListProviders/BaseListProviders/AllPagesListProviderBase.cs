...
public abstract class AllPagesListProviderBase : LimitedListProviderBase, INeedNamespacesListProvider
{
    ...
    protected AllPagesListProviderBase(
    ...
    protected async Task<Result<List<WikiPageModel>>> MakeListBase(
        int limit,
        PropertyFilterOption redirectsFilter,
        PropertyFilterOption langLinksFilter
    )
    {
        return await _pageService.GetAllPages(_settingsService.CurrentApiUrl, Param, _namespaces![0], redirectsFilter, langLinksFilter, limit);
    }
}
