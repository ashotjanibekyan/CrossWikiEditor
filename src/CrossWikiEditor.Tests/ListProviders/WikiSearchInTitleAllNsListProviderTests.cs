...
public sealed class WikiSearchInTitleAllNsListProviderTests : ListProvidersBaseTest<WikiSearchInTitleAllNsListProvider>
{
    ...
    public void SetUp()
    {
        ...
        _sut = new WikiSearchInTitleAllNsListProvider(_dialogService, _pageService, _settingsService) { Param = "start from here" };
    ...
    {
        // arrange
        _pageService.WikiSearch(_userSettings.GetApiUrl(), $"intitle:{_sut.Param}", [], 73).Returns(_expectedPages);
    ...
    {
        // arrange
        _pageService.WikiSearch(_userSettings.GetApiUrl(), $"intitle:{_sut.Param}", [], 73).Returns(new Exception("failed to get pages"));
