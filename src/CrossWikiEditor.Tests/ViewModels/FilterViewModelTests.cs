...
public sealed class FilterViewModelTests : BaseTest
{
    ...
    public void OpenFileCommand_ShouldSetPages_WhenListProviderMakesSuccessful()
    {
        ...
        _systemService.ReadAllTextAsync("some/path/text.txt", Encoding.Default).Returns(text);
    ...
    public void SaveCommand_ShouldCloseWithViewModelValues()
    {
        ...
        _sut.Pages = Fakers.GetWikiPageModelFaker(_settingsService.CurrentApiUrl, _wikiClientCache).Generate(10).ToObservableCollection();
        IDialog dialog = Substitute.For<IDialog>();
        ...
        dialog
            .Received(1)
            .Close(
                Arg.Is<FilterOptions>(filter =>
                    filter.RemoveDuplicates == _sut.RemoveDuplicates
